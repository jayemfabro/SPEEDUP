  ...
  const handleUpdateClass = async (eventId, updateData) => {
    try {
      ...
      if (eventToUpdate.class_type === 'Group' && Array.isArray(eventToUpdate.student_name)) {
        ...
        // Notify admin calendar about the group update
        localStorage.setItem('class_updated', JSON.stringify({
          class_id: eventId,
          updated_at: Date.now(),
          updated_by: 'teacher',
          type: 'group_update'
        }));
        localStorage.setItem('last_class_update', Date.now().toString());
        ...
        // Notify admin calendar about the update
        localStorage.setItem('class_updated', JSON.stringify({
          class_id: eventId,
          updated_at: Date.now(),
          updated_by: 'teacher'
