...
export default function AdminDailyCalendar() {
    ...
    // Real-time updates: Server-side polling for cross-device updates
    useEffect(() => {
        let lastCheckTime = Math.floor(Date.now() / 1000);
        
        const handleClassUpdate = (event) => {
            if (event.key === 'class_updated' && event.newValue) {
                console.log('Admin Daily Calendar detected class update from localStorage');
                // Refresh the calendar data
                setTimeout(() => {
                    fetchClasses();
                }, 500); // Small delay to ensure server update is complete
            }
        };
        ...
        const checkForUpdates = async () => {
            try {
                ...
                if (response.data.has_updates) {
                    console.log('Admin Daily Calendar detected server-side class update, refreshing...');
                    fetchClasses();
        ...
        // Poll every 3 seconds for cross-device updates
        const serverPollingInterval = setInterval(checkForUpdates, 3000);

        // Fallback localStorage polling
        const localPollingInterval = setInterval(() => {
            const lastUpdate = localStorage.getItem('last_class_update');
            if (lastUpdate) {
                const lastUpdateTime = parseInt(lastUpdate, 10);
                const now = Date.now();
                // If there was an update in the last 10 seconds, refresh
                if (now - lastUpdateTime < 10000 && now - lastUpdateTime > 0) {
                    console.log('Admin Daily Calendar detected recent localStorage update, refreshing...');
                    fetchClasses();
                    localStorage.removeItem('last_class_update'); // Clear to avoid repeated refreshes
                }
            }
