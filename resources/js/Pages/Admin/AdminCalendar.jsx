    ...
    useEffect(() => {
        ...
        const handleClassUpdate = (event) => {
            if (event.key === 'class_updated' && event.newValue) {
                console.log('Admin detected class update from localStorage');
        ...
        const checkForUpdates = async () => {
            try {
                ...
                if (response.data.has_updates) {
                    console.log('Admin detected server-side class update, refreshing...');
                    fetchClasses();
                    setRenderKey(prev => prev + 1);
        ...
        const localPollingInterval = setInterval(() => {
            ...
            if (lastUpdate) {
                ...
                if (now - lastUpdateTime < 10000 && now - lastUpdateTime > 0) {
                    console.log('Admin detected recent localStorage update, refreshing...');
                    fetchClasses();
                    setRenderKey(prev => prev + 1);
