...
export default function Teachers() {
    ...
    useEffect(() => {
        fetchTeachers(true); // Show loader on initial fetch
        
        // Set up polling for real-time updates
        const pollInterval = setInterval(() => {
            fetchTeachers(false); // Don't show loader during polling
        }, 5000); // Poll every 5 seconds

        return () => clearInterval(pollInterval);
    }, []);

    const fetchTeachers = async (showLoader = false) => {
        try {
            if (showLoader) {
                setIsLoading(true);
            }
            ...
            console.log(`[${timestamp}] Fetching teachers (showLoader: ${showLoader})`);
            
            const response = await axios.get('/api/admin/teachers');
            console.log(`[${timestamp}] Teachers fetched:`, response.data.length, 'teachers');
    ...
    // Update currentTeacher when teachers array is updated (for real-time updates in modal)
    useEffect(() => {
        console.log('Teachers updated, checking if currentTeacher needs update:', { 
            currentTeacher: currentTeacher?.id, 
            teachersCount: teachers.length 
        });
        
        if (currentTeacher && teachers.length > 0) {
            const updatedTeacher = teachers.find(teacher => teacher.id === currentTeacher.id);
            if (updatedTeacher) {
                const hasChanges = JSON.stringify(updatedTeacher) !== JSON.stringify(currentTeacher);
                console.log('Teacher comparison:', {
                    teacherId: currentTeacher.id,
                    hasChanges,
                    currentName: `${currentTeacher.firstName} ${currentTeacher.lastName}`,
                    updatedName: `${updatedTeacher.firstName} ${updatedTeacher.lastName}`,
                    currentEmail: currentTeacher.email,
                    updatedEmail: updatedTeacher.email
                });
                
                if (hasChanges) {
                    console.log('Updating currentTeacher with fresh data:', updatedTeacher);
                    setCurrentTeacher(updatedTeacher);
                    setModalVersion(prev => prev + 1); // Force modal re-render
                }
            }
        }
    }, [teachers]); // Removed currentTeacher from dependencies to prevent infinite loop

    // Manual refresh function
    const handleRefresh = () => {
        fetchTeachers(true);
        showNotification('Teachers data refreshed', 'success');
    };
    ...
    return (
        <AdminLayout>
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
                    ...
                    <div className="flex flex-col sm:flex-row gap-4 items-center justify-between">
                        ...
                        <div className="flex flex-col sm:flex-row gap-3">
                            <button
                                ...
                                className="w-full sm:w-auto inline-flex items-center justify-center px-4 py-2.5 border border-gray-300 rounded-lg shadow-sm text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-navy-500 transition-all duration-300 whitespace-nowrap"
                            >
                ...
                <DetailsTeacherModal
                    key={`teacher-modal-${modalVersion}-${currentTeacher?.id || 'none'}`}
