...
class TeacherClassController extends Controller
{
    ...
    public function updateStatus(Request $request, $id)
    {
        try {
            ...
            // Broadcast the class update event for real-time updates
            broadcast(new ClassUpdated($class->fresh(), 'status_update', $user));

            // Mark update for cross-device polling
            try {
                \Illuminate\Support\Facades\Http::post(url('/api/class-updates/mark'));
            } catch (\Exception $e) {
                Log::warning('Failed to mark class update: ' . $e->getMessage());
            }
            ...
            try {
                ...
                foreach ($admins as $admin) {
                    NotificationService::createAdminNotification(
                        ...
                        "Teacher {$user->name} updated class status for student {$class->student_name} from '{$oldStatus}' to '{$request->status}'",
                ...
                Log::error('Failed to send admin notification for status update: ' . $e->getMessage());
            }
    ...
    public function updateNotes(Request $request, $id)
    {
        try {
            ...
            // Broadcast the class update event for real-time updates
            broadcast(new ClassUpdated($class->fresh(), 'notes_update', $user));
            ...
            try {
                ...
                foreach ($admins as $admin) {
                    ...
                    NotificationService::createAdminNotification(
                        ...
                        "Teacher {$user->name} updated class notes for student {$class->student_name} from {$oldNotesDisplay} to {$newNotesDisplay}",
                        'teacher_class_update',
                        [
                            'class_id' => $class->id,
                            'teacher_name' => $user->name,
                ...
                Log::error('Failed to send admin notification for notes update: ' . $e->getMessage());
            }
    ...
    public function update(Request $request, $id)
    {
        try {
            ...
            Log::info('Found class: ', [
                ...
                'student_name' => $class->student_name,
                'user_role' => $user->role instanceof \App\Models\UserRole ? $user->role->value : $user->role
            ]);
            ...
            if (!$user->isTeacher() && !$user->isAdmin()) {
                $roleValue = $user->role instanceof \App\Models\UserRole ? $user->role->value : $user->role;
                Log::error('User does not have teacher or admin role: ' . $roleValue);
            ...
            // Broadcast the class update event for real-time updates
            broadcast(new ClassUpdated($class->fresh(), 'update', $user));
            ...
            try {
                ...
                foreach ($admins as $admin) {
                    ...
                    if (isset($updateData['notes']) && $oldData['notes'] !== $updateData['notes']) {
                        $oldNotesDisplay = $oldData['notes'] ?: 'empty';
                        $newNotesDisplay = $updateData['notes'] ?: 'empty';
                        $changes[] = "notes from '{$oldNotesDisplay}' to '{$newNotesDisplay}'";
                    }
                    ...
                    NotificationService::createAdminNotification(
                        $admin->id,
                        'Class Updated by Teacher',
                        "Teacher {$user->name} updated {$changesText} for student {$class->student_name}",
                        'teacher_class_update',
                        [
                            'class_id' => $class->id,
                            'teacher_name' => $user->name,
